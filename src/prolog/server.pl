...
parse_input(board,Board) :- default_board(Board).

parse_input(valid-Board-PlayerChar-X-Y, Type) :- valid_moves(Board,PlayerChar,X,Y,Type).

parse_input(move-Board-PlayerChar-Xi-Yi-Xf-Yf-Type-Player, NewNewBoard-NewEnd) :-
	...
	game_over(NewNewBoard,Player,NewEnd).
